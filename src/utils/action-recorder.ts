...
export class ActionRecorder {
  ...
  async start(page: Page) {
    this.logger.info("🔴 Recording started... Perform your actions in the browser.");

    // Inject a listener into the browser
    await page.exposeBinding("onActionDetected", async ({ frame }, actionData) => {
    ...
    await page.addInitScript(() => {
      window.addEventListener("click", (e) => {
        ...
        const name = target.innerText || target.getAttribute("aria-label") || target.id;
